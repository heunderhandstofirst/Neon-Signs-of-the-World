...
function getBondStrokes(edgeABC, currentCycle) {
  ...
  if (this.edgeABC === 2) {
    ...
    if (this.cycle === 1 || this.cycle === 11) {
      // return(-1)
      return [0, 0, 255]; //0, 0, 255
    }
    if (this.cycle === 2 || this.cycle === 10) {
      return [250, 100, 50];
    }
    return [250, 250, 250];
  }
...
function bulbShape(centerX, centerY, rx, ry, color, alphaFloor, alphaCeiling) {
  for (var i = 0; i < rx; i++) {
    ...
    stroke(
      // color[0] * (botPercentage + random(1 - botPercentage / 2)),
    ...
    if (random() > 0.5) {
      ...
      // line(centerX - i, Splash[0], centerX - i, Splash[1]);
...
class verticalWhiteLinesIgnore {
  constructor(step5, NumLines, rectDim, rectStart) {
